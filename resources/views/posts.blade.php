  {{-- <article class="py-8 max-w-screen-md border-b border-gray-300">
    ...
    <div class="text-base ">
      By
      <a href="/author/{{ $post->author->username }}" class="hover:underline text-gray-500">{{ $post->author->name }}</a> 
      in
      <a href="/category/{{ $post->category->id}}" class="hover:underline text-gray-500">{{ $post->category->name }}</a> | 
      <time datetime="2023-10-01">{{ $post->created_at->diffForHumans() }}</time> 
    </div>
     <p class="my-4">{{ Str::limit($post['body'], 120) }}</p>
